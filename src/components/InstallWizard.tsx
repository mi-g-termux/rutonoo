...
GRANT SELECT,INSERT,UPDATE,DELETE ON settings,products,orders,coupons,categories,newsletter,reviews,users,product_images,product_variant_groups,product_variants TO anon;
GRANT ALL ON settings,products,orders,coupons,categories,newsletter,reviews,users,product_images,product_variant_groups,product_variants TO service_role;
...
CREATE POLICY "anon all" ON products   FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "anon all" ON coupons    FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "anon all" ON categories FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "anon all" ON newsletter FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "anon all" ON reviews    FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "anon all" ON users      FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "anon all" ON product_images         FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "anon all" ON product_variant_groups FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "anon all" ON product_variants       FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "anon all" ON orders     FOR ALL USING (true) WITH CHECK (true);

-- ══════════════════════════════════════════════════
--  PART 2 — Atomic stock function (prevents oversell)
-- ══════════════════════════════════════════════════
...
GRANT EXECUTE ON FUNCTION deduct_stock(TEXT,INT) TO service_role;`}</pre>
                  <button type="button"
                    ...
                    className="absolute top-2 right-2 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white text-[10px] font-bold px-3 py-1 rounded-lg transition-all"
        ...
        {currentStep === 4 && backend === 'firebase' && (
          <div className="flex flex-col gap-5">
            ...
            <details className="text-xs text-gray-600 bg-amber-50 border border-amber-200 rounded-xl overflow-hidden">
              <summary className="cursor-pointer font-semibold text-amber-800 px-4 py-3 flex items-center justify-between hover:bg-amber-100 transition-colors">
                <span>🔥 Firestore Rules — click to expand &amp; copy</span>
                <span className="text-[10px] font-normal text-amber-600">Firestore → Rules tab → Publish</span>
              </summary>
              <div className="px-4 pb-4 pt-2">
                <div className="relative">
                  <pre id="fb-fs-rules" className="bg-slate-900 text-amber-300 text-[10px] font-mono p-3 rounded overflow-x-auto max-h-44 select-all whitespace-pre">{`rules_version = '2';
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /orders/{id}     { allow create: if true; allow get: if true; allow list, update, delete: if request.auth != null; }
    match /newsletter/{id} { allow create: if true; allow read, update, delete: if request.auth != null; }
    match /userPhones/{phoneId} {
      allow read: if false;
      allow create: if request.resource.data.phoneKey == phoneId && request.resource.data.userId is string && !exists(/databases/$(database)/documents/userPhones/$(phoneId));
      allow update, delete: if request.auth != null;
    }
    match /users/{userId} {
      allow read: if true;
      allow create: if request.resource.data.id == userId && request.resource.data.email is string && request.resource.data.name is string && request.resource.data.name.size() > 0 && request.resource.data.passwordHash is string;
      allow update: if request.auth != null || (request.resource.data.id == userId && request.resource.data.email == resource.data.email);
      allow delete: if request.auth != null;
                  ...
                  <button type="button"
                    ...
                    className="absolute top-2 right-2 bg-amber-600 hover:bg-amber-700 active:scale-95 text-white text-[10px] font-bold px-3 py-1 rounded-lg transition-all"
                  >{rulesCopied ? '✅ Copied!' : '📋 Copy'}</button>
                </div>
              </div>
            </details>
            <details className="text-xs text-gray-600 bg-orange-50 border border-orange-200 rounded-xl overflow-hidden">
              <summary className="cursor-pointer font-semibold text-orange-700 px-4 py-3 flex items-center justify-between hover:bg-orange-100 transition-colors">
                <span>🗄️ Storage Rules — click to expand &amp; copy</span>
                <span className="text-[10px] font-normal text-orange-500">Storage → Rules tab → Publish</span>
              </summary>
              <div className="px-4 pb-4 pt-2">
                <div className="relative">
  ...
  match /b/{bucket}/o {
    ...
    match /site/{allPaths=**}       { allow read: if true; allow write: if request.auth != null; }
                  ...
                  <button type="button"
                    ...
                    className="absolute top-2 right-2 bg-orange-500 hover:bg-orange-600 active:scale-95 text-white text-[10px] font-bold px-3 py-1 rounded-lg transition-all"
