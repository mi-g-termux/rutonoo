...
export const AppProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const placeOrder = async (
    ...
    // If any item is out of stock the order is blocked here with a clear
    // user-facing error and the cart stays intact — no half-placed orders.
    ...
    // Uses Firestore runTransaction (fully atomic) or Supabase deduct_stock RPC
    // so two concurrent orders for the last item can never both succeed.
    // If atomicDeductStock throws StockError it bubbles up and blocks the order.
    for (const item of newOrder.items) {
      try {
